...
pub fn count_overlaps(point_criteria: fn((i32, i32, i32, i32)) -> Vec<(i32, i32)>) -> usize {
    let points = read_lines("src/a5/input").flat_map(|line| {
        let vector = line
            .split(" -> ")
            .flat_map(|cord_raw| {
                cord_raw
        ...
        point_criteria((vector[0], vector[1], vector[2], vector[3]))
    });
...
pub fn answer() {
    let answer_a = count_overlaps(|(x1, y1, x2, y2)| {
        if x1 == x2 {
            ...
            (start..=end)
                .map(move |cord| (x1, cord))
                .collect::<Vec<_>>()
        } else if y1 == y2 {
            // vertical
            let (start, end) = (cmp::min(x1, x2), cmp::max(x1, x2));
            (start..=end)
                .map(move |cord| (cord, y1))
                .collect::<Vec<_>>()
        } else {
