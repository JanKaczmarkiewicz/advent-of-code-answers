...
pub fn answer() {
    ...
    let (depth, horizontal_position) = get_operations().fold(
        (0, 0),
        |(depth, horizontal_position), (action, arg)|
            match &action[..] {
    ...
    let (depth_with_aim, horizontal_position_with_aim, _aim) = get_operations().fold(
        (0, 0, 0),
        |(depth, horizontal_position, aim), (action, arg)|
            match &action[..] {
    ...
    println!("Answer to problem 2: {}, {}", depth * horizontal_position, depth_with_aim * horizontal_position_with_aim);
}