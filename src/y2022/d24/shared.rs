...
pub fn answer(destinations: &[Destination]) -> i64 {
    ...
    let mut goals = vec_deque::VecDeque::from_iter(destinations.iter().map(|d| match d {
        Destination::End => end,
    ...
    loop {
        ...
        if valid_paths.contains(&goals[0]) {
            let current_location = goals.pop_front().unwrap();

            if goals.is_empty() {
                return i + 1;
            } else {
                valid_paths.clear();
                valid_paths.insert(current_location);
            }
        }
...
mod tests {
    ...
    fn should_compute_solution() {
        assert_eq!(
            answer(&[Destination::End, Destination::Start, Destination::End]),
            221
        );
    }
}
