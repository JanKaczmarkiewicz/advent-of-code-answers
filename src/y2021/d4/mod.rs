...
fn check_bingo(current_numbers: &[u8], board: &[u8]) -> bool {
...
fn calculate_board_score(board: &[u8], current_bingo_numbers: &[u8]) -> u32 {
    return board
        .iter()
        .filter(|x| !current_bingo_numbers.contains(x))
        .fold(0_u32, |prev, x| prev + *x as u32);
}

fn a(boards: &Vec<Vec<u8>>, all_bingo_numbers: &[u8]) -> u32 {
    let mut i = BOARD_SIZE;
    loop {
        let current_bingo_numbers = &all_bingo_numbers[0..i];
        for board in boards {
            if check_bingo(current_bingo_numbers, board) {
                return all_bingo_numbers[i - 1] as u32
                    * calculate_board_score(board, current_bingo_numbers);
            };
        }
        i += 1;
    }
...
fn b(boards: &Vec<Vec<u8>>, all_bingo_numbers: &[u8]) -> u32 {
    ...
    loop {
        ...
        for board in boards {
            if !check_bingo(current_bingo_numbers, board) {
                return all_bingo_numbers[i] as u32
                    * calculate_board_score(board, &all_bingo_numbers[0..i + 1]);
            };
        }
        i -= 1;
    }
...
pub fn answer() {
    ...
    println!(
        "Answer to problem 4: {}, {}",
        a(&boards, &all_bingo_numbers),
        b(&boards, &all_bingo_numbers)
    );
}
