...
fn a() -> u32 {
    let lines = read_lines("src/a3/input").collect::<Vec<_>>();
    ...
    let it_gamma = get_result().fold(0, |prev, (i, cond)| {
        prev + if cond { 2_u32.pow(i as u32) } else { 0 }
    });
    let it_epsilon = get_result().fold(0, |prev, (i, cond)| {
...
fn find(
    matrix: Vec<Vec<u32>>,
    depth: usize,
    pick_matrix: fn(Vec<Vec<u32>>, Vec<Vec<u32>>) -> Vec<Vec<u32>>,
) -> u32 {
    if matrix.len() == 1 {
        matrix[0]
            .clone()
            ...
            .rev()
            .enumerate()
            .fold(0, |prev, (i, cond)| prev + cond * 2_u32.pow(i as u32))
    } else {
        let (zeros, ones): (Vec<_>, Vec<_>) = matrix.into_iter().partition(|e| e[depth] == 0);
...
fn b() -> u32 {
    ...
    // TODO: remove clone
    let oxygen = find(lines.clone(), 0, |zeros, ones| {
    ...
    let scrubber = find(lines, 0, |zeros, ones| {
        if zeros.len() > ones.len() {
            ones
        } else {
            zeros
